...
**Debug port**: launching with `AIRLOOM_DEBUG_SOCKET=/path/to.sock ./run` (or `scripts/debug-run`, which kills stray instances and picks `$XDG_RUNTIME_DIR/airloom-debug.sock` by default) opens a local 0600 Unix socket — newline-delimited JSON, driven with `scripts/debug-client [--socket PATH] CMD [JSON_ARGS]` — so an agent can drive and inspect the running app. Commands: `ping`/`version` (version, git-describe build id, pid, `debug: true`), `eval` (JS in the webview), `pinch` (synthesize begin/change/end), `tap {x,y}` (synthetic pointerdown/pointerup/click via `window.Airloom.debugTap`), `search {query}` (`window.Airloom.debugSearch`), `key {key}` (`window.Airloom.debugKey`), `state` (single-round-trip snapshot from `window.Airloom.debugState`), `screenshot {path?}` (webview-only PNG via `WebView.get_snapshot`; base64 if no path), `quit`. See `airloom/debugport.py` — payload validation for every command (`tap`/`search`/`key`/`state`/`screenshot`/`quit`/`version`; `ping` implicitly) lives in its GTK-free `validate_command()` table, unit-tested in `tests/test_debugport.py` without a display. A debug launch runs with `Gio.ApplicationFlags.NON_UNIQUE` so it never just activates an already-running instance, and is visibly distinct: window title `"Airloom · DEBUG"` and a red header bar (`self.debug_mode` in `app.py`, decided once at startup) — the taskbar icon override is best-effort only since GNOME/Wayland ties it to the desktop entry. Trackpad pinch is intercepted as a capture-phase `Gtk.GestureZoom` in `app.py` (claimed before WebKitGTK's internal page-scale gesture can consume it) and forwarded over the bridge as `pinch` events; `window.visualViewport.scale` staying 1.0 is the probe that page-scale zoom stayed off.
